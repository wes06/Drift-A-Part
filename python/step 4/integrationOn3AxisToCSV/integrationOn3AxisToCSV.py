...
with open('../mockCSVGenerator-rdmUnsigned/mockCSVData.csv', newline='') as in_csvfile, open('positions.csv', 'w') as out_csvfile:
	...
	fieldnames = ['posX', 'posY', 'posZ', 'velX', 'velY', 'velZ', 'vectLength', 'timestamp']
	...
	for row in IMUData:
		...
		vectLength = math.sqrt(velX**2 + velY**2 + velZ**2)

		if abs(velX) > peakVelX:
			peakVelX = abs(velX)
		if abs(velY) > peakVelY:
			peakVelY = abs(velY)
		if abs(velZ) > peakVelZ:
			peakVelZ = abs(velZ)
		if abs(vectLength) > peakVectLength:
			peakVectLength = abs(vectLength)
...
print('\tPeak speed:\t\t\t %.2f m/s' % peakVelX)
...
print('\tPeak speed:\t\t\t %.2f m/s' % peakVelY)
...
print('\tPeak speed:\t\t\t %.2f m/s' % peakVelZ)
print('\tElapsed distance:\t %.2f m\n' % distZ)

print('\tPeak combined speed:\t %.2f m/s' % peakVectLength)
